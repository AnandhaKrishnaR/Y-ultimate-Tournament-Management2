...
function AddPlayerForm() { 
  ...
  const handleAddPlayer = async (e) => {
    ...
    if (!selectedTeam || !selectedTournament || !username) {
      setError('Please select a team, a tournament, and enter a player username.');
      return;
    }
    ...
    try {
      ...
      setSuccess(`Player '${username}' successfully added to the team!`);
      setUsername(''); 
    } catch (err) {
      console.error('Registration failed:', err.response.data);
      setError(`Failed to add player: ${JSON.stringify(err.response.data)}`);
    }
  };

  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-xl font-semibold text-gray-800 mb-4">Add Player to Roster</h3>
      <form onSubmit={handleAddPlayer} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700">Player's Username:</label>
          <input
            type="text"
